...
def run_query(attribute_name, offset, limit):
    ...
    print(f"{cho_url}, headers = {headers}, params = {query}")
    response = requests.request("GET", cho_url, headers = headers, params = query)

    if requests.codes.ok != 200:
        print(f"warning {requests.codes.ok} will try again in a few seconds")
...
def do_cho_queries(attribute_name, output_json_file, passed_offset_start, limit_max, batch_size):
    ...
    print(f"totalAttributes={totalAttributes}")
    ...
    print(f"writing to {output_json_file} offset {off_set_start}")
    ...
    print(f"{off_set_start}, {limit_max}, {offset_size}")
    for offset in range(off_set_start, limit_max, offset_size):
        print(f"{offset} of {totalAttributes}")

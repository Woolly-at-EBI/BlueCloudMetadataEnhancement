...
def get_term_regex_dict():
    ...
    term_regex_dict = {
        ...
        "test_regex": "([+-]?",
        "altitude": "((0|((0\.)|([1-9][0-9]*\.?))[0-9]*)([Ee][+-]?[0-9]+)?)|((^not collected$)|(^not provided$)|(^restricted access$)|(^missing: control sample$)|(^missing: sample group$)|(^missing: synthetic construct$)|(^missing: lab stock$)|(^missing: third party data$)|(^missing: data agreement established pre-2023$)|(^missing: endangered species$)|(^missing: human-identifiable$))"
    }
    return term_regex_dict


def test_reqexs(term_regex_dict):
    """"""

    def is_valid_reqex(test_reqex):
        try:
            re.compile(test_reqex)
            is_valid = True
        except re.error:
            is_valid = False
    ...
    def simple_test(test_regex, test_string):
        if not is_valid_reqex(test_regex):
    ...
    for term in term_regex_dict:
        ...
        if term == "simple_int":
            ...
            if is_valid_reqex(term_regex_dict[term]):
